...
1.  Not all callers can call the same SVTYPES - Lumpy and WHAM are
    unable to identify insertions. WHAM does not call translocations.
2.  With the exception of INS (Lumpy and WHAM) and TRA (WHAM), all
    callers are able to call 68% or more of TPs in the truth set.
3.  Of all the callers tested, GRIDSS and Manta were the most performant
    over all SVTYPES
4.  For DEL, DUP, INV and TRA most of the difference between caller seen
    are in how well the program controls FP - precision differences.
...
1.  Use upsetR to look at the overlaps between the truth set and all
    callers tested for a given SV type
...
1.  11/13 (85%) of translocations were identified by all callers
    (excluding WHAM)
